...
const logger = {
  info: (m: string) => process.stderr.write(`[ssdb] ${m}\n`),
  warn: (m: string) => process.stderr.write(`[ssdb WARN] ${m}\n`),
  error: (m: string) => process.stderr.write(`[ssdb ERROR] ${m}\n`),
};
...
export class SuperScalarDbService {
  ...
  /**
   * Set the counter to a specific value. Used by the migration path
   * that moves data out of CLN datastore — the plugin reads its prior
   * value once from datastore and pushes it here, after which this
   * service owns the counter.
   */
  public setIidCounter(value: number): void {
    if (!Number.isInteger(value) || value < 0 || value > 0xFFFFFFFF) {
      throw new Error(`setIidCounter: value must be u32 (0..2^32-1), got ${value}`);
    }
    this.db
      .prepare(
