...
import {
  Factory, FactoryCreateOptions, FactoryCreateResponse, FactoryRotateResponse,
  FactoryCloseResponse, FactoryForceCloseResponse, FactoryCheckBreachResponse,
  FactoryTriggerCeremonyResponse, FactoryJoinRequestResponse,
  FactoryBrowseHostResponse, FactoryIncomingJoinsResponse,
} from '../types/factories.type';
import {
  RendezvousSettings,
  LspIdentity,
...
export class FactoriesService {
  ...
  static async createFactory(
    ...
    if (options) {
      ...
      if (options.defer_signing != null) params.defer_signing = options.defer_signing;
    }
    return HttpService.clnCall('factory-create', params);
  }

  /**
   * Trigger the MuSig2 signing ceremony on a factory previously created with
   * defer_signing=true. Sends CEREMONY_START (0x0145) + FACTORY_PROPOSE
   * (0x0100) to every accepted joiner; the existing wire protocol drives
   * the rest of the dance.
   */
  static async triggerCeremony(
    factoryInstanceIdHex: string,
    options?: { force?: boolean; deadlineBlock?: number },
  ): Promise<FactoryTriggerCeremonyResponse> {
    const params: Record<string, any> = { factory_instance_id_hex: factoryInstanceIdHex };
    if (options?.force != null) params.force = options.force;
    if (options?.deadlineBlock != null) params.deadline_block = options.deadlineBlock;
    return HttpService.clnCall('factory-trigger-ceremony', params);
  }

  /**
   * Client-side: send a join request to a target LSP for a specific factory.
   * Carries contribution_sats (how much the client is bringing) and an
   * idempotent request_id generated by the plugin.
   */
  static async joinRequest(
    peerNodeId: string,
    factoryInstanceIdHex: string,
    contributionSats: number,
  ): Promise<FactoryJoinRequestResponse> {
    return HttpService.clnCall('factory-join-request', {
   ...
   */
  static async cancelJoin(requestId: string): Promise<{ ok: boolean }> {
    return HttpService.clnCall('factory-cancel-join', { request_id: requestId });
  }

  /**
   * Client-side: query an LSP's public factory list. Returns the factories
   * the LSP is currently advertising (capacity, slots open/total, lifecycle,
   * fee rate). Round-trips a FACTORY_INFO_REQUEST/RESPONSE pair over LN.
   */
  static async browseHost(peerNodeId: string): Promise<FactoryBrowseHostResponse> {
    return HttpService.clnCall('factory-browse-host', { peer_node_id: peerNodeId });
  }

  /** LSP-side: list pending join requests, optionally filtered to one factory. */
  static async incomingJoins(factoryInstanceIdHex?: string): Promise<FactoryIncomingJoinsResponse> {
    const params: Record<string, any> = {};
    if (factoryInstanceIdHex) params.factory_instance_id_hex = factoryInstanceIdHex;
    return HttpService.clnCall('factory-incoming-joins', params);
  }

  /** LSP-side: reject a joiner from a specific factory. */
  static async kickJoiner(factoryInstanceIdHex: string, peerNodeId: string, reason?: string): Promise<{ ok: boolean }> {
    const params: Record<string, any> = {
      factory_instance_id_hex: factoryInstanceIdHex,
      peer_node_id: peerNodeId,
    };
    if (reason) params.reason = reason;
    return HttpService.clnCall('factory-kick-joiner', params);
  }
